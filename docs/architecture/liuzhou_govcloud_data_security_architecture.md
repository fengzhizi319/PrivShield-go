...
**统计**：✅ **25** 项代码闭环 · 🟡 **4** 项仍有残余（外部依赖或部署侧动作）。

#### 已闭环项（✅ 代码级无缺口，仅待外部签核/演练/部署确认）

P0-1（fail-closed 门禁）、P0-2（逐字段矩阵 + 默认拒绝）、P0-3（空密钥拒绝 + enc:v2:）、P0-4（strict storage 三服务）、P0-6（存证客户端绑定）、P0-7（BFF 代理白名单）、P0-8（归档链路 + 留存红线）、P1-1（定级驱动算子）、P1-4（ICD 双路径一致性）、P1-5（词表唯一源）、P1-7（HKDF-SM3 KDF）、P1-8（双 DSN 编排 + 告警）、P1-9（per-endpoint 熔断）、P2-1（幻影变量清除）、P2-2（YAML 配置绑定）、P2-3（主链路 SM3）、P2-4（验真 reason 枚举 + gRPC 同步）、P2-6（裸标识清零）、P2-7（error_class 重试判定）、P2-10（flusher 参数化）、P2-11（连接池环境变量）、P2-12（postgresql.conf 入库）。

#### 仍有残余的活跃项（🟡）

| 编号 | 残余缺口 | 阻塞方 |
|:---:|---|---|
| **P0-5** | ① 默认端点仍为环回明文 `http://localhost:8000/...`，生产必须显式覆盖为 https；② 引擎分类响应保留 `Value` 回显字段（本地闭环用）；③ 真实 LLM 出网白名单属局方 ACL | 部署方（端点覆盖）+ 局方（出网 ACL） |
| **P1-2** | 未引入 SM2 签名、密码模块未取得商用密码产品认证；「可归责溯源/不可否认」表述不得对外发布 | 局方密码测评机构（外部排期） |
| **P1-3** | 标准文件已加载但**为纯声明**（不参与分类决策）；真实 NER 模型权重未交付（CGO/ONNX 桩） | 算法交付方（NER 模型） |
| **P1-6** | 「审计员只读/运维只写」的制度性权责签核属局方管理动作；engine-go 网关 `/metrics` 仍无鉴权（网关设计即不终止凭据，默认环回监听） | 局方（制度签核）+ 架构裁定（网关） |
