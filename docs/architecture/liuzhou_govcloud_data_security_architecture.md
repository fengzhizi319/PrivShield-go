...
| **4. 检验检查** | 实验室检验、HIV、精神类特异指标 | **L4** (指标) / **L5** (基因与重症病种取值) | 重度污名化、社会排斥 | **区间化 + 彻底抹平（`purge_categories` + `replacement_labels`）** ✅ | 二次授权 + 特异性遮盖 🔴（未实装） | 裸哈希 (无法做科研统计) |
...
| `redaction_strategy`（`:226`） | 抹平/泛化**策略路由**与占位标签映射 | ④用药/处置（替换文本防反推） |

其中**防止组合推理反向识别**的核心机制是 `composite_rules`：单条记录同时出现诊断、疾病、基因、突变等 ≥2 个特征字段时直接提升为 L5，而非依赖单字段掩码。

**敏感病种差异化治理策略（真实配置键与 ICD 区间）**

* **彻底抹平（`redaction_strategy.purge_categories`）** ✅：当前配置的抹平范畴为 `HIV_AIDS`、`PSYCHIATRIC_DISORDER`、`GENETIC_DEFECT`、`STD_VENEREAL`，出域前将病名、指标、特异性药物彻底剔除，并以 `replacement_labels` 置换为**不含原始敏感词的抽象标签**（如 `HIV_AIDS` ➔ `[L5-IMMUNODEFICIENCY-…]`、`STD_VENEREAL` ➔ `INFECTIOUS_DISEASE`），避免替换文本本身成为侧信道；
...
| `icd10_code` | ICD-10 疾病编码 | **L4** | `RedactICD10Code`：L5 编码**整值抹空**、L4 编码**换范畴码**、非高危原样 | `B20.900` ➔ `""`；`A51.000` ➔ `[L4-ICD_INFECTIOUS]`；`C34.900` ➔ `[L4-ICD_NEOPLASM]`；`I10.x00` ➔ 原样 | ✅ 已实装（⚠️ 非原表述的「保留前 3 位类目」泛化；`rules.go:149-198`） |
| `diagnosis_name` | 临床诊断中文名称 | **L4** | 含高危词 → 范畴标签；**不含高危词时命中 `name` 子串 → 按中文姓名掩码** | `急性心肌梗死` ➔ `[L4-SEVERE_ORGAN_DAMAGE]`；`原发性高血压` ➔ `原****压` | 🔴 **误用姓名掩码**：首尾字保留使慢病名仍可推断，且非确定性泛化 |
...
| `family_history` | 家族遗传病史 | **L4** | 值含高危词才 `RedactMedicalText`，否则原样 | `父亲因恶性肿瘤去世，一弟患重度精神分裂症` ➔ `父亲因[L4-MALIGNANT_NEOPLASM]去世，一弟患[L5-PSYCHIATRIC_DISORDER]` | 🟡 **条件性生效**（未命中词表即明文） |
...
| `diagnosis_name` | 主要疾病诊断 | **L4** | 含高危词→范畴标签；否则命中 `name`→姓名掩码 | `重度精神分裂症` ➔ `[L5-PSYCHIATRIC_DISORDER]`；`2型糖尿病` ➔ `2***病` | 🔴 同医保 `diagnosis_name` |
...
| **G-07** | **Layer-3 原值外送关闭或改造** | 🟡 代码侧改造已入库（原值不再出网）：`buildPrompt` 已删除 `字段值: %s`，只提交**字段名 + 值形态指纹**（`engine-go/internal/dynclassification/llm_client.go:144` `ShapeOf` 是升级路径唯一读原值的函数、`:555` 起的 prompt 组装仅注入 `值形态指纹(已去标识化，无原值)`、`:109-128` 指纹不可逆推约束）；出网前对请求体做**原值包含性自检**，命中即拒发并计数（`:278,779`）；端点方案强制校验：`https/wss` 放行、明文 `http://` 仅环回放行，跨主机明文需 `PRIVACY_LLM_ALLOW_INSECURE_HTTP_ENDPOINT=true` 显式豁免（`:53,331-369`）；Layer-3 默认关闭（`config/privacy.yaml:30`）。⚠️ **残余**：① 默认端点字面量仍是环回明文 `http://localhost:8000/v1/chat/completions`（`:60`、`engine-go/internal/service/service.go:95`），生产启用时**必须显式覆盖为 https 端点**，否则环回放行成立；② 引擎分类响应结构体仍保留 `Value` 回显字段（本地闭环用），未做协议级删除；③ 出网 ACL 与真实模型端点属局方交付物 | ① 保持 `PRIVACY_LLM_ENABLE=false`（默认）时，出网 ACL 证明无可对外大模型端点可达即可签核；② 启用时提供 prompt 组装单元测试断言原值不出现在请求体中（`llm_client_test.go:115` `TestBuildPrompt_ExcludesOriginalValue`、`:203` `TestLLMClient_ClassifyShape_RequestBodyHasNoRawValue`）；③ 出具 `https://` 端点配置快照 + `/ops/diagnostics` 的 `transport_secure:true` 证据；④ 豁免变量若置 `true`，须书面说明网段受控性与替代链路保护措施 | **P0-5** |
| **G-08** | **定级驱动算子闭环** | REST `/api/hub/dispatch` 不再接受调用方自证的 `operation`（含 `none`），统一由定级结果推导；gRPC 侧去除定级缺失时静默回退 `L2` 的行为，改为拒绝 | ① 提交 `operation:"none"` 对受管数据源返回校验错误；② 同一数据集经 REST 与 gRPC 两路径得到**一致**的脱敏结果 | **P1-1 / P0-2** |
| **G-09** | **多副本与租约档位落地**（如需多副本） | 显式配置 `SERVICE_HUB_PG_DSN` 与 `AUDIT_LOG_PG_DSN`，并验证 `FOR UPDATE SKIP LOCKED` 路径生效；单副本 SQLite 形态须在部署文档中明确「无多副本语义」 | ① `ClaimNext` 在两副本并发下无重复领取；② SQLite/内存模式下 `ErrLeaseNotSupported` 不出现在生产日志 | **P1-8** |
| **G-10** | **暴露面收敛与哈希链日巡检** | 各服务监听地址与云安全组收敛为受控子网，`bff-go`/`app-lz` 不得公网可达；将 `GET /api/audit/chain/verify` 接入日巡检并出具对账日报；pprof/`/metrics` 生产关闭或加 `ops:admin` | ① 端口扫描结果与《允许暴露清单》一致；② 连续 ≥7 天验真日报无断链；③ 未授权访问 `/debug/pprof` 返回 401/404 | **P0-7 / P1-6 / P2-8** |
| **G-11** | **存证留存期与归档落地** | `AUDIT_LOG_RETENTION_DAYS` 设为 **0（关闭删除）或 ≥1095**；改造归档链路使 `AUDIT_LOG_ARCHIVE_DIR` 真正被消费（压缩加密后转储 OSS/对象存储）；将清理由 `DELETE` 改为**时间分区 + `DROP PARTITION`**，并解除 `snapshots` 的 `ON DELETE CASCADE` 或改为归档后再脱落 | ① 部署配置快照显示留存不短于 3 年；② 造一批超期数据后验证**先出现归档文件、再发生数据移除**，且归档包可解密验真；③ 跨 90 天边界的哈希链验真仍能通过（历史区间可回放） | **P0-8** |
...
| **P0-5** | 声明「大模型零接触政务明文」；事实为**引擎 Layer-3 把字段原值明文拼入 prompt 并以 HTTP 外送** | `engine-go/internal/dynclassification/llm_client.go:228-239` 的 `buildPrompt` 含 `字段值: %s`；默认端点为明文 `http://localhost:8000/v1/chat/completions`（`:41`）；`PRIVACY_LLM_ENABLE` 未置 `true` 时不启用（`service.go:75`） | 一旦现场开启 Layer-3，**L4/L5 原值将离开受控域**，与 §1.2 第一原则直接冲突 | ① prompt 仅提交字段名 + 值形态指纹（长度/字符类别/是否含数字），去除原值；② 强制 HTTPS/mTLS 并禁止 `http://` 端点；③ 出站目的地址纳入白名单 ACL + 单元断言原值不在请求体 | **G-07** |
...
| **P1-3** | 声明「三层 AI 自动分类分级 + 国标对齐」；事实为**Layer-2 为 CGO 桩、规则库未编码 GB/T 43697 映射，能力边界即规则完备性** | `onnx_ner.go:174` 注明完整实现待引入 CGO；`cuda_onnx_ner.go:99` 直接返回 `stub ONNX runtime: CGO binding not available`；`rules/standards/` 仅 `sc_health_db51.yaml`/`gd_health.yaml`/`jrt0197.yaml`，**无国标映射文件**；柳州地方标准亦无专属规则集 | 未显式声明的字段只能得到正则级识别 + Safety Floor 托底（实际 `MinLevel=public`），**语义泛化能力缺位**；地方标准落地不足 | ① 补齐 GB/T 43697 与柳州市地方标准规则集（含重要数据/核心数据目录标记）；② 若要宣称 AI 定级，须交付并部署可用 NER 模型；③ 文档以「规则库驱动的自动分类分级」表述替代「AI 自动分类分级」 | 规则集覆盖率报告 + 未声明字段抽样本的定级准确率验收 |
...
| **P1-6** | 声明「审计侧只写、权责分离」；事实为**应用层无只写约束、无审计员角色模型，验真与写库共用 fail-open 鉴权** | `pkg/store/postgres/audit.go:138-189` 建表为标准可读写表，无账号权限分离逻辑；鉴权为全局 `middleware.Auth(APIKey)` 且空值放行（`pkg/middleware/auth.go:23-29`），仅覆盖 `/api/*`，`/metrics` 无鉴权 | 「业务端无权删改审计日志」完全依赖部署侧，产品本身不提供该能力 | ① 提供只写数据库账号与权限脚本（`REVOKE UPDATE/DELETE`）+ 启动自检当前账号权限；② 引入角色化账户（审计员只读、运维只写）；③ 核验专区路由白名单化 | **G-10** + 以只写账号执行 `DELETE` 必须被数据库拒绝 |
...
**统计**：✅ **21** 项 · 🟡 **8** 项 · 🔴 **0** 项（P0 **7✅/1🟡** · P1 **6✅/3🟡** · P2 **8✅/4🟡**）。

#### （1）P0 级 —— 上线阻塞项

| 编号 | 状态 | 落地证据（`file:line`） | 残余缺口 |
|:---:|:---:|---|---|
| **P0-1** | ✅ | 统一 fail-closed 启动门禁 `pkg/config/security.go:69`（`ValidateFailClosed`）+ 空白名单致命错误 `:24`（`ErrMTLSWhitelistRequired`）；六个服务全部接入：`engine-go/internal/config/config.go:145`、`service-hub/internal/config/config.go:233`、`datasource-mgr/internal/config/config.go:131`、`audit-log/internal/config/config.go:193`、`console/bff-go/internal/config/config.go:360`、`console/app-lz/bff-go/internal/config/config.go:185`；生产编排安全开关默认 `true`（`deploy/docker-compose/docker-compose.prod.yml:130,141,148,248,260,371,449,514`）；`*_REQUIRE_TLS=true` 而 `*_TLS_ENABLED=false` 组合直接拒绝启动（`docker-compose.prod.yml:8`） | 无代码缺口。G-02/G-03 须由局方以真实证书与 CN 授权清单签核 |
| **P0-2** | ✅ | 逐字段处置矩阵入库：`privacy-go-sdk/medical/fields.go:498`（医保 18 字段）、`:548`（康养 27 字段），长文本走 L4 实体剥离、体征数值走 DP 加噪；未列入字段**默认拒绝**：`engine-go/internal/service/privacyconfig.go:75-88`（`unlisted_field_policy` / `unlisted_min_level` + 具名可审计策略 `unlistedFieldFloor`）、`:306`（分类侧下限）、`:212`（词表外取值不再回落 `LevelPublic`）；Safety Floor 默认 `internal`（`engine-go/internal/dynclassification/safety_floor.go:88`）；通用掩码路径二次兜底（`service.go:1476`）；生效值快照经 `/ops/diagnostics` 可审计（`service.go:1273`、`privacyconfig.go:348`）；行为断言 `privacy-go-sdk/medical/pipeline_test.go:136`（未知字段默认拒绝）、`:170`（未列入字段定级为 L3） | 覆盖口径为「示范数据源契约字段 ∪ 历史 SDK 规格名」**并集**（`fields.go:503,553`），故矩阵条数多于 18/27；真实样本抽测准确率仍属 §11.3 G-08 局方验收动作 |
| **P0-3** | ✅ | 空密钥不再静默降级：`pkg/crypto/envelope.go:53`（`ErrEmptyKey`）、`:107-110`（`EncryptString` 空密钥直接拒绝，**不返回明文**）；明文透传通道被彻底消除：`:57`（`ErrUnencryptedValue`）+ `:148-158`（`DecryptString` 判定顺序为 空串 → `enc:v2:` HKDF 路径 → `enc:v1:` 历史 SHA-256 路径 → **无前缀一律报错**，调用方无法把「剥离前缀」当成降级出口）；当前写入统一 `enc:v2:`（`:39`），`enc:v1:`（`:36`）仅供存量读取（`:154-155,185`）；启动强制要求密钥 `services/audit-log/internal/config/config.go:201-202`（`RequireEncryptionKey: true`） | 存量 `enc:v1:`/明文快照的迁移重签需在真实审计库上执行（本地无 PG）；DEK/KEK 两级密钥包裹待密码与合规组方案（见 P1-7） |
| **P0-4** | ✅ | `AUDIT_LOG_STRICT_STORAGE` 默认 `true`（`services/audit-log/internal/config/config.go:171`）；SQLite 完整性校验失败 → `log.Fatalf` 阻断（`cmd/server/main.go:72`），strict 模式下拒绝启动且**禁止自动重建**（`:407`，对比非 strict 分支 `:409`）；存证写失败上抛为请求失败；`service-hub` 纳入同一档位（本轮补齐）：`services/service-hub/internal/config/config.go:89,172`（`SERVICE_HUB_STRICT_STORAGE` 默认 true，专用变量优先于全局 `STRICT_STORAGE`）+ `cmd/server/main.go:514`（已配置 `SERVICE_HUB_PG_DSN` 而探测失败 → 上抛错误退出，**不再静默回退到无租约语义的 SQLite/内存**）、`:116`（启动即打印 `strict_storage`/`lease_capable` 存储档位）；`datasource-mgr` 同源档位 `internal/config/config.go:95` + `internal/handlers/data_provider.go:169`；三服务编排显式声明（`deploy/docker-compose/docker-compose.prod.yml:401,464,539`）；回归 `services/service-hub/cmd/server/strict_storage_test.go`、`internal/config/config_test.go`（TestStrictStorageDefault）、`services/audit-log/internal/config/config_test.go:111` | 无代码缺口。G-05/G-06 签核 + 现场演练「拔库」路径由运维执行 |
| **P0-5** | 🟡 | prompt 去原值：`engine-go/internal/dynclassification/llm_client.go:144`（`ShapeOf` 为升级路径唯一读原值函数）、`:573`（prompt 仅注入 `值形态指纹(已去标识化，无原值)`）、`:109-111`（指纹不可逆推约束）、`:77-83`（`Shape` 载荷定义）；明文 `http://` 端点默认拒绝、需 `PRIVACY_LLM_ALLOW_INSECURE_HTTP_ENDPOINT=true` 显式豁免（`:53,319,336,367`）；出网前原值包含性自检计数（`:278,779`） | ① 默认端点仍为环回明文 `http://localhost:8000/...`（`:60`），生产 Helm/compose **必须显式覆盖为 https 端点**，否则环回放行仍成立；② 引擎分类响应结构体保留 `Value` 回显字段（供本地闭环），未做协议级删除（改动面涉及对外契约）；③ 真实 LLM 出网白名单属局方 ACL |
| **P0-6** | ✅ | 新建存证客户端 `services/service-hub/internal/audit/client.go`（`New:136`、`RecordOutbound:226`、`RecordOutboundEvidence:275`、失败分类 `FailureClass:287`）；`audit` 阶段真实提交并绑定指纹：REST 侧 `internal/handlers/handlers.go:656`（提交失败即 `task.Status="failed"` 并落 `error_class`，见 `:666-679`）、gRPC 侧 `internal/grpcserver/server.go:686`；配置项 `SERVICE_HUB_AUDIT_LOG_URLS`（➔ `SERVICE_HUB_AUDIT_HTTP` ➔ 编排默认值）、`SERVICE_HUB_AUDIT_LOG_API_KEY`（➔ `AUDIT_LOG_API_KEY`）、`SERVICE_HUB_AUDIT_LOG_TLS_{ENABLED,CERT_FILE,KEY_FILE,CA_FILE}`（`internal/config/config.go:125-136,245-264`）；未配置存证端点时：全环回监听**直接拒绝启动**（`ErrAuditEndpointRequired` `:23`），远程绑定则放行进程但**每一条出域任务在 audit 阶段失败**，不存在「已出域无存证」路径 | 无代码缺口。跨节点时钟一致性与 audit-log 侧幂等键去重需现场联调确认 |
| **P0-7** | ✅ | BFF 透明代理改**默认拒绝 + 方法/路径白名单**：`console/bff-go/internal/handlers/handlers.go:1643`（`microserviceProxyAllowlist` 按 hub/datasource/audit 分组）、`:1681`（`records`/`sample`/`/api/v1/*` 第二道硬拦截）、`:1711`（查表判定）、`:1868`（转发前强制校验）、`:1875`（拒绝响应）；转发层自身拒绝非规范化/编码穿越路径（`internal/microservices/client.go:66`）；代理层全量留痕（`handlers.go:1792`）；`CONSOLE_API_KEY` 空值启动失败（`internal/config/config.go:360`）；回归 `internal/handlers/microservice_proxy_allowlist_test.go:107-240`（含 `../`、`%2e%2e`、双斜线、`limit=1` 等绕过样本） | 无代码缺口。《允许暴露清单》须局方书面确认，白名单变更走该清单 |
| **P0-8** | ✅ | 留存默认 `0（不删）`：`services/audit-log/internal/config/config.go:161`；`>0` 时强制 ≥ **1095 天**红线（`:14-16`、`:220`）且必须同时配置归档目录与加密密钥（`:224,227`）；**先归档后删除**链路 `services/audit-log/internal/archive/archive.go`（`ArchiveAndCleanup:108` 逐页读出 → SM4-GCM+gzip 段 + SM3 行链 manifest → 回读验真 → 才删该页，缺密钥/缺目录/存储不支持均拒绝删除 `:52-58`）；解除 `snapshots ... ON DELETE CASCADE` 依赖改为显式按页删除 | 分区化以运维脚本交付（见 P2-9），存量 PG 实例的分区迁移未在真实库执行 |

#### （2）P1 级 —— 上线前必须整改项

| 编号 | 状态 | 落地证据（`file:line`） | 残余缺口 |
|:---:|:---:|---|---|
| **P1-1** | ✅ | REST 不再由调用方决定是否脱敏：`services/service-hub/internal/handlers/handlers.go:581`（任何携带数据的任务必须过引擎）、`:624`（算子由定级推导）；gRPC 去除静默 `L2` 回退、定级缺失即失败：`internal/grpcserver/server.go:248,585`（调用方 `operation` 只能**上调**保护强度）、`:375,624,855`（`EffectiveOperation` 收口）；引擎分类响应补 `level` 字段 | 无代码缺口。G-08 双路径一致性复测由局方执行 |
| **P1-2** | 🟡 | 哈希链**密钥化**：`pkg/store/audit_hash.go:42-43`（`SM3-HMAC:v1` 版本标签）、`:55`（`SetAuditChainKey` 注入局方托管密钥）、`:100`（写侧 `HMAC-SM3(key, "SM3-HMAC:v1|"+9 要素前映像)`）、`:130-136`（验真按「密钥化 → 无密钥 SM3-UTC → SHA-256-UTC → …」候选顺序，**存量行仍可验真**）、`:217`（重签工具据此升级存量）；密钥入口 `AUDIT_LOG_HASH_KEY`（`services/audit-log/internal/config/config.go:58-59`）；`scripts/prod/repair_hash_chain.go` 已纳入模块可被 CI 编译；文档口径统一为「完整性防篡改」 | **未引入 SM2 签名、密码模块未取得商用密码产品认证**（外部机构排期，不可代码闭环）；故「可归责溯源/不可否认」表述仍不得对外发布，密钥托管凭证待局方提供 |
| **P1-3** | 🟡 | 新增国标映射 `rules/standards/gbt43697.yaml`；Layer-2 明确以规则桩交付并**运行时自证能力边界**：`engine-go/internal/service/service.go:1261,1271`（`ner_available:false` 经 `/ops/diagnostics` 上报）；文档以「规则库驱动的自动分类分级」替换「AI 自动分类分级」 | `gbt43697.yaml` **当前无引擎加载路径**（仅标准资产入库，待与 `rules/domains` 装载链打通）；真实 NER 模型权重未交付（CGO/ONNX 桩，`onnx_ner.go:174`、`cuda_onnx_ner.go:99`） |
| **P1-4** | ✅ | SDK 硬编码 ICD 等级对齐 YAML 唯一事实源：`privacy-go-sdk/medical/rules.go:176`（L4 组）、`:189`；双路径等级一致性 CI 断言 `privacy-go-sdk/medical/icd10_yaml_consistency_test.go`；流水线侧 `services/service-hub/tests/pipeline_operation_test.go` | 无代码缺口。全量 ICD 抽样比对以 CI 断言覆盖，抽样规模扩大属测试数据治理 |
| **P1-5** | ✅ | 词表唯一源 `rules/taxonomies/default.yaml`；一致性门禁入 `make check`（`Makefile:68,72` → `scripts/check_taxonomy_consistency.sh`），校验 `pkg/validation` / `pkg/naming` / `engine-go` 三处派生口径 | 无代码缺口 |
| **P1-6** | 🟡 | 只写数据库账号与权限脚本 `deploy/sql/audit_writeonly_role.sql`（`REVOKE UPDATE/DELETE`）+ 启动自检 `has_table_privilege`（`services/audit-log/cmd/server/main.go:472` 注释所述、`:386-391` 在 `DBWriteOnly=true` 时执行，自检失败拒绝启动）；**权责分离角色模型**落地：`pkg/middleware` 的 `AuthWithRoles` + `services/audit-log/internal/handlers/handlers.go:77`（只读端点集 `auditReadOnlyEndpoints`）、`internal/config/config.go:132,212`（`AUDIT_LOG_READER_API_KEY` 必填且**必须与写 Key 不同**，相同即拒绝启动）；compose/k8s/env 模板全链路注入（`deploy/docker-compose/.env.prod.example`） | `/metrics` 端点仍无鉴权（G-10 余项）；「审计员只读 / 运维只写」的**制度性权责签核**属局方管理动作 |
| **P1-7** | ✅ | KDF 升级为 **HKDF-SM3（RFC 5869，Extract-then-Expand，杂凑取 GM/T 0004-2012 SM3）**：`pkg/crypto/envelope.go:74-98`（`DeriveKeyHKDF`/`hkdfExtract`/`hkdfExpand`）、`:44-48`（逐记录 16 字节随机 salt + 用途绑定 `info`）、`:60-64`（旧 `SHA-256(secret)[:16]` 降级为**仅供 v1 解密**）；前缀参与 AAD（`:38`） | DEK/KEK 两级包裹与 KMS/HSM 对接仍待密码与合规组方案（与 P1-2 同源外部依赖）；对外统一口径改为「HKDF-SM3 + SM4-GCM 认证加密」 |
| **P1-8** | ✅ | 生产编排默认注入双 DSN：`deploy/docker-compose/docker-compose.prod.yml:397`（`SERVICE_HUB_PG_DSN`）、`:540`（`AUDIT_LOG_PG_DSN`），且以 `${PG_USER:?...}` 强制变量非空；未启用 PG 时启动显式告警「多副本能力不可用」（`services/service-hub/cmd/server/main.go:526`、探测 `:486`） | 两副本并发压测「无重复领取」需真实 PG + 多副本环境（G-09 未实测） |
| **P1-9** | ✅ | 熔断改 **per-endpoint**：`pkg/agent/client.go:63`（`breakers map[string]*cbState` 节点维度状态）、`:443`（`breakerFor(endpoint)`）；`PickEndpoint`（`:399`）**剔除自身熔断器开启的节点**，全池冷却时快速失败；`reportCircuitState` 上报**真实失败节点**（`:812-817,858,890`）；重试换节点；`client_test.go` 保持 3/3 轮询与错误串断言不变 | `privshield-gateway`（P2C-EWMA per-node）是否纳入部署仍待架构裁定（第十二章「明确不闭环项」） |

#### （3）P2 级 —— 治理与增强项

| 编号 | 状态 | 落地证据（`file:line`） | 残余缺口 |
|:---:|:---:|---|---|
| **P2-1** | ✅ | 幻影变量清除（`DATASOURCE_MGR_DB_PATH` 等）+ 端口冲突修正（`console/app-lz/bff-go/Dockerfile:41` 删除 `50055`、`deploy/docker-compose/docker-compose.app-lz.yml:192` 记录原漂移来源）；新增一致性门禁 `scripts/check_orchestration_env_consistency.sh` 并入 `make check`（`Makefile:68,77-78`） | 无代码缺口 |
| **P2-2** | ✅ | `config/privacy.yaml` 的 `classification.*` / `safety_floor.*` 绑定进运行时：`engine-go/internal/service/privacyconfig.go:52-88`（YAML 结构体绑定）、`:210`（`min_level` 解析）、`:306`、热更新 `service.go:1605`、生效快照 `service.go:525,1273` | 无代码缺口。`enable_ner` 有意**只解析不绑定**（桩实现，绑定会造成「已启用」假象，见 `privacyconfig.go:64-65`） |
| **P2-3** | ✅ | 主链路指纹改国密：`engine-go/internal/service/service.go:822,825`（`crypto.SumSM3Hex` 计算 `input_hash`/`output_hash`）；接口契约与文档口径同步 | 无代码缺口。`hash_sm3` 显式原语入口保留 |
| **P2-4** | 🟡 | 验真响应补机器可读结论：`pkg/store/store.go:313-346`（`ChainReasonOK/LegacyHashed/TamperedPayload/HashMismatch/BrokenChain/MissingPrev/MissingRecords` + `Reason` 字段）；断点判定 `pkg/store/postgres/audit.go:696-725`；遍历改**规范重放序** `ORDER BY seq ASC, timestamp ASC, id ASC`（`postgres/audit.go:666,797`、`sqlite/audit.go:479,651`、memory 与 repairchain 同步） | **gRPC 侧 `reason` 字段未透出**：需 `protoc` 重新生成 `*.pb.go`，本轮仅在 REST 契约落地（已在 §6.4 标注差异） |
| **P2-5** | 🟡 | engine 侧由「仅日志」升级为**指标 + 日志双通道**并与其他四服务口径齐平：`engine-go/cmd/privshield-agent/main.go:118`（`naming.SetObserver(namingObserver{metrics: engineMetrics})`）、`:312-325`；指标定义 `engine-go/internal/observability/metrics.go`（`privshield_api_alias_requests_total` / `privshield_datasource_normalize_errors_total`，`ConstLabels module=privshield-agent`，与 `pkg/metrics` 同名同标签） | `console/bff-go` **不存在 naming 卡点**（全仓检索无 `naming.` 调用点），注册空观测器将是无消费者的死代码，故该项以「口径说明」关闭而非埋点；该 BFF 的标识校验在 `console/app-lz/bff-go`（已注册 `main.go:57`） |
| **P2-6** | ✅ | 业务代码裸标识清零：`engine-go/internal/service/service.go:848`（`naming.API1Yibao, naming.DSYibao`）、数据源路由 `:634,650,773`；诊断快照键值改常量（`engine-go/internal/service/privacyconfig.go:371-372`）；`datasource-mgr` gRPC 桩 `internal/grpcserver/server.go:156,176` 改 `naming.DSMock3/DSMock4` | 唯一保留的裸字面量集合是**归一化自测的入站别名样本**（`console/app-lz/bff-go/internal/runner/runner.go:522-529` 与注释），其语义即「非规范输入」，改常量会削弱测试，属明示例外 |
| **P2-7** | ✅ | 字符串猜测全部废除，改为**失败点一次性判定 + 落库 + 扫描只读分类**：`pkg/store/store.go:49`（`Task.ErrorClass`）、schema `pkg/store/postgres/schema.go`（`error_class TEXT DEFAULT ''` + `ALTER TABLE ... IF NOT EXISTS`）、`pkg/store/sqlite/init.go`（列 + 存量库迁移分支）；读写贯通 `postgres/tasks.go`、`postgres/leased.go`（三处 `FailLease` 分支携带 `failure.ErrorClass`）、`sqlite/tasks.go`；判定唯一入口 `services/service-hub/internal/retry/retry.go:83`（`IsRetryableClass`）、`:91`（`Classify(err, bias)` 依 `errors.Is/As` 与 sentinel 身份，含 `BiasConservative`/`BiasDownstream` 两种偏置）；五个失败点接入（`handlers.go` panic/shutdown/engine/缺级/存证，`grpcserver/server.go` 同）；旧 `isRetryableError(errMsg string)` 已删除；回归 `internal/retry/retry_test.go`、`cmd/server/retry_test.go`（含「文本像可重试但分类为空则不重试」陷阱用例） | 行为差异（reviewer 须知，非缺口）：① `server shutting down` 类失败现在**可重试**（受 3 次上限约束）；② 旧版本落库的 `error_class=''` 存量行**永不自动重试**（fail-safe，需人工或重签工具处置） |
| **P2-8** | 🟡 | 建立**可复现**仓内基准：`make bench`（`Makefile`，输出含 commit/branch/dirty_files/Go 版本/CPU 型号/核数/内核/UTC 时间指纹）+ 报告入库 `docs/reports/benchmark_baseline.md`（33 Benchmark × `-count=3`，median-of-3 + 分配量 + 离散度标注）；关键实测：单字段定级 49–58 ns（0 分配）、10 字段整行脱敏 722 ns、DP 家族 0 分配、流式 CSV 较物化 **2.64× 快 / −37% 内存** | **§9 的吞吐/时延/IOPS 仍为公式推算**：端到端压测（REST/gRPC 全链路 QPS、P99）、多副本容量曲线、真实 NER/GPU 推理、生产 TLS 握手开销**均未实测**（无政务云专区环境），§9.1/§9.6「未实测」标注保持 |
| **P2-9** | 🟡 | 按月 RANGE 分区与整段清理运维脚本交付：`deploy/sql/audit_partition.sql`（`:149` `PARTITION BY RANGE (timestamp)`、`:183,:423` 自动滚动建分区、`:371-376` `DETACH`+`DROP` 两步清理（PG16 无单步 `DROP PARTITION`）、`:280` 约束换名守卫）；脚本内**明示三项语义代价**（主键改 `(id,timestamp)`、放弃 `CASCADE` 外键、`DROP PARTITION` 绕过应用「先归档后删除」守卫 → 不提供一键删除） | 应用侧清理仍走 `DELETE`（`pkg/store/postgres/audit.go` 按页删除）；**存量表迁移未在真实 PG 执行**（停写窗口 + DBA 动作），新装/迁移窗口按脚本执行 |
| **P2-10** | ✅ | flusher 全参数环境入口：`services/audit-log/internal/config/config.go:72-77`（`AUDIT_LOG_FLUSH_BATCH_SIZE`/`_INTERVAL_MS`/`_QUEUE_SIZE`/`_ENQUEUE_TIMEOUT_MS`/`_MAX_STAGED`/`_CLOSE_TIMEOUT_MS`）、`:152-157` 读取、`cmd/server/main.go:448-451` 覆盖 `DefaultConfig()` 并回显生效值 | 无代码缺口。高并发档位的取值需 P2-8 端到端压测后定标 |
| **P2-11** | ✅ | `AUDIT_LOG_PG_MAX_CONNS` / `AUDIT_LOG_PG_MIN_CONNS`（`internal/config/config.go:68-69,145-146`）传入 `postgres.Config{MaxConn,MinConn}`（`cmd/server/main.go:378-382`）；§9.7 虚构的 `DB_MAX_OPEN_CONNS/IDLE/LIFETIME` 已从文档移除 | 无代码缺口。`pg_stat_activity` 实测需真实 PG |
| **P2-12** | ✅ | `deploy/postgres/postgresql.conf` 入库并保持 `synchronous_commit = on`（`:57` 说明「返回成功即 WAL 已 fsync」）；三套 compose 均挂载并显式 `-c` 指向（`docker-compose.prod.yml:716`、`docker-compose.yml:766`、`docker-compose.dev.yml:396`） | 最终 `postgresql.conf` 仍属外部 DBA 交付物，须由其按专区实际 IO 复核参数 |
...
5. **P1-1 先于 G-08 验收**：双路径一致性测试必须在 REST 侧不再接受调用方自证 `operation`（含 `none`）之后进行，否则测试本身可被绕过。
