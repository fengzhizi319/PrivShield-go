...
3. **高保真模拟数据库**：内置 19 字段医保就医结算数据与 27 字段康养体征随访档案；
...
graph TD
    ...
    subgraph MockDatasourceMgr ["Mock Datasource Mgr 微服务 (:8083 / :50053)"]
        ...
        subgraph EmbeddedDatasets [内置模拟数据集 (对齐 DB51/T 2989—2023)]
            DS1[(API 1: ds_yibao 医保就医结算 19字段)]
...
|---|---|---|---|---|---|
| **API 1** | 医保数据源 (`ds_yibao`) | `GET /api/datasources/ds_yibao/records` | `GetYibaoData` | 19 字段 | 包含 `insurance_settlement_id`, `person_id`, `id_card`, `diagnosis_name`, `icd10_code`, `total_amount` |
