...
`services/audit-log` 是 PrivShield 平台的企业级脱敏合规审计与不可篡改存证微服务。模块提供 **REST (HTTP/HTTPS :8084，支持 mTLS / TLCP 国密) + gRPC (mTLS/Plain :50054)** 双协议接入，支持全量隐私原语与分类治理事件存证、8 要素增强 SHA-256 防篡改校验、快照对账与多维度合规报告生成。

---

## 核心功能特性

- **双协议接入**：提供标准 REST API（端口 `:8084`，支持明文 / TLS 1.3 mTLS 双向认证 / GM/T 0024 TLCP 国密双证书）与高性能 gRPC 接口（端口 `:50054`，供调度流水线高并发写入）；
- **零信任 mTLS 与公钥固定**：gRPC 与 HTTP 通道均支持 TLS 1.3 双向证书认证与客户端公钥固定（Public Key Pinning），以及 TLCP 国密专线；
