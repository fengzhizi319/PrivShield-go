...
| | **抗拒绝服务与速率限制** | 内置令牌桶高并发限流中间件（默认 100 RPS，突发 200，32 分片并发安全）；并发调度信号量控制（`taskSem` 最大并发 10 个重量级任务）。 | **完全满足** |
...
| **D (Denial of Service) 拒绝服务** | 恶意高频并发打垮调度中枢与底层脱敏引擎 | 32 分片并发安全令牌桶限流（100 RPS / 200 Burst）；`taskSem` 并发信号量限流（最多 10 并发重任务）；超时熔断保护。 |
