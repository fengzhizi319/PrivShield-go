// Command server is the entry point for the mock-datasource module.
// Command server 是模拟数据源模块（mock-datasource）的程序主入口。
//
// 本服务在 PrivShield 架构中扮演“数据源资产与敏感特征模拟服务”角色，
// 作为数据提供者（Data Provider）仅向数盾调度中枢 service-hub 输出结构化医保、康养及测试数据源；
// 前端控制台与 BFF 网关不直连本服务，统一经 service-hub 编排调度，同时支持 HTTP(S) REST 与双向认证 gRPC (mTLS) 双协议接入。
//
// ==============================================================================
// Architecture & Traffic Flow / 系统架构与流量图：
// ==============================================================================
//
//	┌────────────────────────┐   HTTP(S) REST (明文 / TLS 1.3 mTLS 双向认证) (:8083)    ┌────────────────────────────────────────┐
//	│ service-hub 数据调度中枢 │ ────────────────────────────────────────────────────────▶ │ mock-datasource 模拟数据源服务          │
//	│   【唯一直接调用方】     │   gRPC (明文 / TLS 1.3 mTLS 双向认证) (:50053)          │ - HTTP(S) REST: :8083 (mTLS/明文)      │
//	└────────────────────────┘ ────────────────────────────────────────────────────────▶ │ - gRPC: :50053 (mTLS/明文)             │
//	                                                                                       │ - 提供 yibao/kangyang 模拟多源数据     │
//	                                                                                       └────────────────────────────────────────┘
