...
const translations: Record<Language, Record<string, string>> = {
  'zh-CN': {
    ...
    'dataApi.desc': '模拟外部数据申请方接入 service-hub 调度中枢。点击“申请”触发完整流通链路：外部发起申请 → service-hub 调度内部拉取 → engine 分类脱敏 → audit-log 存证 → 仅脱敏合规数据出域交付给外部申请方（严格遵守原始明文数据不出域原则）。',
    'dataApi.flowTitle': '全链路会话流转 (5 阶段)',
    ...
    'dataApi.dataDiff': '合规交付数据 (已脱敏)',
    'dataApi.showRaw': '查看交付 JSON',
    'dataApi.hideRaw': '收起 JSON 视图',
    'dataApi.rawRecords': '原始明文记录 (域内受限)',
    'dataApi.sanitizedRecords': '脱敏交付记录',
    'dataApi.auditWritten': '审计存证 ID',
    'dataApi.rawIsolated': '原始数据受域保护不出域',
    'dataApi.rawIsolatedDesc': '零信任架构约束：原始明文数据仅限调度中枢与隐私计算引擎内部安全流转，严禁出域；外部数据申请方仅接收治理脱敏后的合规数据。',
    ...
    // Auth & RBAC
    'auth.title': '数盾 · 调度之眼',
    'auth.subtitle': '登录以访问数据服务调度中枢控制台',
    'auth.loginTab': '登录',
    'auth.registerTab': '注册',
    'auth.username': '用户名',
    'auth.usernamePlaceholder': '请输入用户名（3-32字符）',
    'auth.password': '密码',
    'auth.passwordPlaceholder': '请输入密码（至少6位）',
    'auth.displayName': '显示名称',
    'auth.displayNamePlaceholder': '可选，如“张三”',
    'auth.role': '角色',
    'auth.roleUser': '常规用户',
    'auth.roleAdmin': '管理员',
    'auth.loginBtn': '登录',
    ...
    'auth.loading': '处理中...',
    'auth.logout': '退出',
    'auth.footer': '数据服务调度中枢控制台',
    'auth.errorGeneric': '操作失败，请重试',
  },
  'en-US': {
    // App Header & Nav
    ...
    'dataApi.desc': 'Simulating external data consumer accessing service-hub. Click "Apply" to trigger the governance chain: external request → service-hub internal dispatch & fetch → engine classification & masking → audit-log store → only sanitized compliant data exits domain (raw data never leaves domain).',
    'dataApi.flowTitle': 'Full-Chain Session Flow (5 Stages)',
    ...
    'dataApi.dataDiff': 'Compliant Delivered Data (Sanitized)',
    'dataApi.showRaw': 'View Delivered JSON',
    'dataApi.hideRaw': 'Collapse JSON View',
    'dataApi.rawRecords': 'Raw Plaintext (Domain-Protected)',
    'dataApi.sanitizedRecords': 'Delivered Sanitized Records',
    'dataApi.auditWritten': 'Audit Entry ID',
    'dataApi.rawIsolated': 'Raw Data Kept In-Domain',
    'dataApi.rawIsolatedDesc': 'Zero-trust policy: raw plaintext records are processed internally within service-hub and engine-go and never exit the domain. External consumer only receives sanitized data.',
    ...
    // Auth & RBAC
    'auth.title': 'PrivShield · Eye of Hub',
    'auth.subtitle': 'Sign in to access the Service Hub Console',
    ...
    'auth.displayNamePlaceholder': 'Optional, e.g. "John Doe"',
    'auth.role': 'Role',
    'auth.roleUser': 'Regular User',
    'auth.roleAdmin': 'Administrator',
    'auth.loginBtn': 'Sign In',
    'auth.registerBtn': 'Register & Sign In',
    'auth.loading': 'Processing...',
    'auth.logout': 'Logout',
    'auth.footer': 'Service Hub Console',
    'auth.errorGeneric': 'Operation failed, please retry',
  },
};
