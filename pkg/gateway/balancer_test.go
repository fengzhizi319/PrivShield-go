// Package gateway 测试套件
//
// ==============================================================================
// 【测试套件设计目标与覆盖范围】
// 本测试文件验证负载均衡器全部策略与熔断器状态机的核心功能：
//  1. 【熔断器状态机】：验证 Closed→Open（连续失败触发熔断）、Open→HalfOpen（冷却期自动降级）、
//     HalfOpen→Closed（探测成功恢复）、HalfOpen→Open（探测失败重新熔断）的完整状态转换；
//  2. 【P2C 策略】：验证随机选择可用节点、偏好低负载节点、结合 EWMA 延迟偏好低延迟节点；
//  3. 【RoundRobin 策略】：验证轮询均匀分配、无锁并发安全性；
//  4. 【LeastConn 策略】：验证选择连接数最少的节点；
//  5. 【WeightedRoundRobin (Nginx SWRR) 策略】：验证精确比例分配、平滑交替、等权重均匀、并发安全；
//  6. 【WeightedRandom 策略】：验证近似概率分布（允许 ±3% 偏差）、单节点始终选中；
//  7. 【熔断器 + 策略集成】：验证熔断节点被跳过、全部熔断时仍返回节点（防雪崩）；
//  8. 【构造函数与默认值】：验证权重缺失/零值默认为 1；
//  9. 【InFlight 原子计数】：验证增减操作正确性、并发安全性、永不减到负数。
// ==============================================================================
